...
return (
  <section className="py-0 pb-14">
    <div className="max-w-screen-xl mx-auto px-4 md:px-8">
      <div className="mt-12">
        <ul className="grid sm:grid-cols-2 gap-8 md:grid-cols-3">
          {team.map((item, i) => (
            <li key={i}>
              <div onClick={() => openModelBox(i + 1)} className="w-full h-60 sm:h-52 md:h-56">
                <Image src={item.avatar} alt="" className="w-full h-full object-cover object-center shadow-md rounded-xl" />
              </div>
            </li>
          ))}
        </ul>
